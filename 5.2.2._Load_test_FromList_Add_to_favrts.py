...
# Эта функция закрывает страницу 'napoveda', если она открыта,
#  и возвращается к основной странице с запущенным тестом.
...
def close_help_page_if_open():
    ...
    try:
        ...
        for tab in tabs:
            ...
            # Если текущая вкладка содержит URL 'napoveda', то закрываем её
...
# Функция очищения полей перед следующм запуском теста
def clear_fields():
...
def fill_odkud_with_random_location(field_id, field_name):
    while True:
        # Получаем случайное место из списка
